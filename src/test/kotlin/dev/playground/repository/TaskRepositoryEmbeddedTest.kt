...
class TaskRepositoryEmbeddedTest {
    ...
    @Test
    @Sql("classpath:test-data.sql")
    fun `when task is deleted then check for the size of the list`() {
        //Given
    ...
    fun `when description is queried then check if description already exists`() {
        val doesDescriptionExist1 = objectUnderTest.doesDescriptionExist("second test to do")
        val doesDescriptionExist2 = objectUnderTest.doesDescriptionExist("feed the dog")
